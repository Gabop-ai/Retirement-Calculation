...
st.set_page_config(page_title="Retirement Master Simulator", page_icon="📈", layout="wide")
st.title("📈 Household Retirement & Tax Simulator")
st.markdown("Model your dual-income retirement trajectory, test return rates, and view tax impacts interactively.")
...
with st.sidebar.expander("💰 Starting Portfolio Assets", expanded=False):
    ...
    ul_initial_cash_value_each = st.number_input("UL Cash Value (Each Policy)", value=62000)

with st.sidebar.expander("📥 Annual Contributions", expanded=False):
    user_rrsp_annual_contrib = st.number_input("User Annual RRSP Contribution", value=32000)
    user_tfsa_annual_contrib = st.number_input("User Annual TFSA Contribution", value=7500)
    spouse_rrsp_annual_contrib = st.number_input("Spouse Annual RRSP Contribution", value=15000)
    spouse_tfsa_annual_contrib = st.number_input("Spouse Annual TFSA Contribution", value=7500)

with st.sidebar.expander("🛡️ Pensions & Guaranteed Income", expanded=False):
    cpp_oas_annual = st.number_input("Gov CPP & OAS Combined (Age 65+)", value=53800)
    spouse_company_pension = st.number_input("Spouse Company Pension (Starts at Spouse Retirement)", value=35000)

def get_rrif_minimum_pct(age):
    rates = {71:0.0528, 72:0.0540, 73:0.0553, 74:0.0567, 75:0.0582, 76:0.0598, 77:0.0617, 78:0.0636, 79:0.0658, 80:0.0682,
             81:0.0708, 82:0.0738, 83:0.0771, 84:0.0808, 85:0.0851, 86:0.0900, 87:0.0955, 88:0.1021, 89:0.1099, 90:0.1192}
    if age < 71: return 0.0
    return rates.get(age, 0.20)

def estimate_alberta_tax(gross_income):
    if gross_income <= 0: return 0.0
    if gross_income <= 58523: tax = gross_income * 0.22
    elif gross_income <= 117045: tax = 12875 + (gross_income - 58523) * 0.305
    elif gross_income <= 181440: tax = 30724 + (gross_income - 117045) * 0.38
    else: tax = 55194 + (gross_income - 181440) * 0.42
    return round(tax, -2)
...
for age in range(start_age, end_age + 1):
    ...
    if is_retired:
        if spouse_age_current >= retirement_age_spouse: pensions += spouse_company_pension
        if age >= 65: pensions += cpp_oas_annual
    ...
    results.append({
        "Age": age, "Spouse Age": spouse_age_current,
        "Portfolio Drawdown": round(required_draw, 0), "Pensions": round(pensions, 0),
        "Total Gross Income": round(total_gross, 0), "Est. Tax Paid": round(est_tax, 0),
        "Effective Tax Rate (%)": eff_tax_rate, "Total Household Portfolio": total_portfolio
    })
    ...
    if age < end_age:
        ...
        s_rrsp *= (1 + r); s_tfsa *= (1 + r); s_mf *= (1 + r); ul_user *= (1 + r); ul_spouse *= (1 + r)
...
st.dataframe(df_display.style.format({
    "Spouse Age": "{:.0f}", "Portfolio Drawdown": "${:,.0f}", "Pensions": "${:,.0f}",
    "Total Gross Income": "${:,.0f}", "Est. Tax Paid": "${:,.0f}", "Effective Tax Rate (%)": "{}%",
    "Total Household Portfolio": "${:,.0f}"
}), use_container_width=True)

st.subheader("📈 Visual Trends")
fig, axes = plt.subplots(nrows=2, ncols=1, figsize=(10, 8), sharex=True)
axes[0].plot(df_master["Age"], df_master["Total Household Portfolio"] / 1e6, label=f"Total Portfolio ($M) @ {r*100:.1f}% Return", color="navy", linewidth=2.5)
axes[0].set_ylabel("Portfolio Value ($ Millions)")
axes[0].grid(True, linestyle=":", alpha=0.6)
axes[0].legend(loc="upper left")

axes[1].bar(df_master["Age"], df_master["Pensions"] / 1e3, label="Pensions", color="skyblue")
